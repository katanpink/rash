...
func _physics_process(delta):
	self.rotate(Vector3(0, 1, 0), deg_to_rad(0.65))
	...
	# Handle Jump.
	if Input.is_action_just_pressed("ui_accept") and is_on_floor() or Input.is_action_just_pressed("ui_accept") && dblj == true:
	...
	if Input.is_action_pressed("run")  && !Input.get_axis("move_left", "move_right") == 0:
		RUN = true
	else:
		RUN = false
	...
	# Reverse controls if rotated more than 180 degrees
	if input_dir != 0:
		input_dir *= sign(-self.rotation.y)
